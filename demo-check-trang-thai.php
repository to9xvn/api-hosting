...
$url = "https://dailysieure.net/dichvu/api-hosting/check.html?email=$email&key=$key&tranid=$tranid";
...
if ($response !== false) {
    ...
    if (!empty($data)) {
        ...
        if ($data['status'] == "2") {
            ...
            echo "Nội dung: " . $data['info']['noidung'] . "\n";
            echo "Ngày hết hạn: " . date('Y-m-d H:i:s', $data['info']['exp']) . "\n";
        }
    } else {
